...
if (isAuth()){ // Il faut être autorisé à utiliser Roboto
	...
	if (m.startsWith(prefix+'github ') || m.replace(/https:\/\/github\.com\/[A-z0-9-]+\/[A-z-]+/, "") == "" ||  m.replace(/https:\/\/github\.com\/[A-z0-9-]+\/[A-z-]+\//, "") == "") {
		...
		request({url: "https://api.github.com/search/repositories?q="+q, headers: {'User-Agent': '*'}}, (error, request, body) => {
			let json = JSON.parse(body),
				...
				ownerUrl = json.items[0].owner.html_url,
	...
	if (m.startsWith(prefix+"code")){
		if (m.replace(prefix+"code") != " " && m.replace(prefix+"code") != "") {
			msg.delete().then(() => {
				if (isBruh() || isAdmin()){
